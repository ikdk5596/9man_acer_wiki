...
async function remove() {
  ...
  busy.value = true; error.value = ''
  try {
    const client = await getClient(); const failures = await client.remove(target, uid)
  ...
  <section class="guides-ui" aria-label="개인 공략 게시판">
    <header class="guide-topbar">
      ...
      <div class="guide-actions">
        ...
        <a class="guide-button primary" :href="guideHref('/guides/write')">공략 쓰기</a>
      </div>
    </header>
    ...
    <template v-if="id">
      ...
      <article v-if="guide" class="guide-article">
        <div class="guide-meta"><span class="guide-badge">{{ guide.category }}</span><span v-if="guide.status === 'draft'" class="guide-badge">나만 보는 초안</span><span>{{ guide.nickname }}</span><time>{{ date(guide.updatedAt) }}</time></div>
        <h2 class="guide-title">{{ guide.title }}</h2>
        <div v-if="guide.authorId === session.uid" class="guide-actions">
          <a class="guide-button" :href="guideHref('/guides/write', guide.id)">수정</a>
    ...
    <template v-else>
      ...
      <div v-else class="guide-list" :aria-busy="loading">
        ...
        <a v-for="row in rows" :key="row.id" :href="guideHref('/guides/', row.id)" class="guide-list-row">
          <div><span class="guide-badge">{{ row.category }}</span><span v-if="row.status === 'draft'" class="guide-badge">초안</span><strong>{{ row.title }}</strong></div>
          <div class="guide-list-meta"><span>{{ row.nickname }}</span><time>{{ date(row.updatedAt) }}</time></div>
        </a>
        <p v-if="loading" role="status" class="guide-empty">공략을 불러오는 중…</p>
